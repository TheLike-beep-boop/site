// ========= 1. Identifică stingătorul din link (?id=...) =========
const params = new URLSearchParams(window.location.search);
const stingatorId = params.get("id");

const fields = ["tipStingator", "producator", "locAmplasare", "firma", "dataVerificare", "serieStingator"];
...
function loadData() {
    db.collection("stingatoare").doc(stingatorId).get().then((doc) => {
        if (!doc.exists) {
            document.body.innerHTML = "<h2 style='padding:40px;text-align:center;color:#D4121A'>Nu există date pentru acest stingător.</h2>";
            return;
        }
        const data = doc.data();
        fields.forEach(f => {
...
// ========= 3. Calculează Valid / Invalid pe baza datei de expirare =========
function updateValidityBadge() {
    ...
    if (today > expDate) {
        status.innerHTML = '<i class="fa-solid fa-xmark"></i> Stingator Expirat';
        setBoxColor("#D4121A");
    } else {
        status.innerHTML = '<i class="fa-solid fa-check"></i> Stingator Valid';
        setBoxColor("#51C323");
    }
}
...
document.getElementById("saveBtn").addEventListener("click", () => {
    ...
    db.collection("stingatoare").doc(stingatorId).set(updatedData, { merge: true })
        .then(() => {
